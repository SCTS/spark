...
class MapOutputTrackerSuite extends SparkFunSuite {
  ...
  test("master register shuffle and fetch") {
    ...
    tracker.registerMapOutput(10, 0, MapStatus(BlockManagerId("a", "hostA", 1000), -1,
        Array(1000L, 10000L)))
    tracker.registerMapOutput(10, 1, MapStatus(BlockManagerId("b", "hostB", 1000), -1,
        Array(10000L, 1000L)))
  ...
  test("remote fetch") {
    ...
    masterTracker.registerMapOutput(10, 0, MapStatus(
      BlockManagerId("a", "hostA", 1000), -1, Array(1000L)))
    masterTracker.incrementEpoch()
    slaveTracker.updateEpoch(masterTracker.getEpoch)
  ...
  test("remote fetch below akka frame size") {
    ...
    masterTracker.registerShuffle(10, 1)
    masterTracker.registerMapOutput(10, 0, MapStatus(
      BlockManagerId("88", "mph", 1000), -1,  Array.fill[Long](10)(0)))
  ...
  test("remote fetch exceeds akka frame size") {
    ...
    (0 until 100).foreach { i =>
      masterTracker.registerMapOutput(20, i, new CompressedMapStatus(
        BlockManagerId("999", "mps", 1000), -1, Array.fill[Long](4000000)(0)))
    }
    val sender = mock(classOf[RpcEndpointRef])
    when(sender.address).thenReturn(RpcAddress("localhost", 12345))
    val rpcCallContext = mock(classOf[RpcCallContext])
...
//    masterTracker.stop() // this throws an exception
  ...
  test("getLocationsWithLargestOutputs with multiple outputs in same machine") {
    ...
    tracker.registerMapOutput(10, 0, MapStatus(BlockManagerId("a", "hostA", 1000), -1,
        Array(2L)))
    tracker.registerMapOutput(10, 1, MapStatus(BlockManagerId("a", "hostA", 1000), -1,
        Array(2L)))
    tracker.registerMapOutput(10, 2, MapStatus(BlockManagerId("b", "hostB", 1000), -1,
        Array(3L)))
